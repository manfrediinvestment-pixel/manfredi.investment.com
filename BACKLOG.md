...
| 32 | Worker `manfredi-noticias` (patrón cache-aside de `manfredi-calendario`, KV + cron cada 30 min, TTL 45 min): agrega Ámbito/Infobae/Cronista/Bloomberg Línea (Argentina) y Yahoo Finance/Investing/Seeking Alpha/The Economist (Wall Street), devuelve `{argentina, wallstreet, updated}` en `/noticias`. Código escrito y validado (parser RSS probado contra los 8 feeds reales; `wrangler dev` no puede levantar en este sandbox por límite de memoria del runtime `workerd`). `index.html` ya está conectado (`loadDailyReports` pide al Worker en paralelo y usa sus noticias si responde ok, con fallback a `reports/*.json` si falla o todavía no está deployado — validado con Playwright mockeando ambos casos). **Falta que el usuario cree el KV namespace** (`wrangler kv namespace create NOTICIAS_KV`), pegue el id real en `workers/noticias/wrangler.toml` y haga el deploy — no hay credenciales de Cloudflare en este entorno. | Alta | Pendiente (deploy) |
