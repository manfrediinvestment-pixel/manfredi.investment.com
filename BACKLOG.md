...
| 32 | Worker `manfredi-noticias` (patrón cache-aside de `manfredi-calendario`, KV + cron cada 30 min, TTL 45 min): agrega Ámbito/Infobae/Cronista/Bloomberg Línea (Argentina) y Yahoo Finance/Investing/Seeking Alpha/The Economist (Wall Street), devuelve `{argentina, wallstreet, updated}` en `/noticias`. Código escrito y validado (parser RSS probado contra los 8 feeds reales, `wrangler dev` no puede levantar en este sandbox por límite de memoria del runtime `workerd`). **Falta que el usuario cree el KV namespace** (`wrangler kv namespace create NOTICIAS_KV`), pegue el id real en `workers/noticias/wrangler.toml` y haga el deploy — no hay credenciales de Cloudflare en este entorno. Falta también conectar `index.html` a este endpoint nuevo: hoy `#reportes` sigue leyendo `noticias` desde `reports/*.json` (generado por `update_reports.py`, cron semanal); ese cambio de fuente queda para cuando se implemente el rediseño de la sección (item 33). | Alta | Pendiente |
| 33 | Rediseño de `#reportes` (Informes & Reportes): sacar el chrome de "ventana de terminal" (puntitos semáforo rojo/amarillo/verde), agrandar los números de variables (dólar blue/oficial/MEP/CCL, Merval, S&amp;P/Nasdaq/Dow, T10Y, commodities) a formato dashboard con más aire, y espaciar las noticias de Argentina/Wall Street para que no compitan por el mismo espacio comprimido. Plan de estructura a mostrar y aprobar en chat antes de implementar. | Alta | Pendiente |
