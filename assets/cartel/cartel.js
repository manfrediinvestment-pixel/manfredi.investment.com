...
function wait(S,t,ms){return new Promise(function(r){setTimeout(function(){r(t===S.v);},ms);});}
...
function deckStart(){head=0; cards.forEach(function(c){c.classList.remove('out');}); layout(true); clearInterval(deckTimer); deckTimer=setInterval(step,2600);}
...
async function runPortfolio(){
  ...
  for(var n=0;n<ASSETS.length;n++){
    ...
    await Promise.all([
      ...
      anim(function(e){tiles.forEach(function(tile,i){tile.querySelector('strong').textContent=RLAB[i][1](from[i]+(to[i]-from[i])*e);});},800)
    ]);
    cur=to; curS=toS;
    if(!await w(STATIC?0:1100))return;
  }
  itxt.style.color='var(--ink2)'; itxt.innerHTML='';
  var msg='<strong>Concentración en tecnología del 71%</strong>: alta para un perfil moderado. Con un beta de 1,03 tu cartera se mueve casi igual que el mercado, y el 71% en tus tres primeras posiciones pesa más que cualquier ratio.';
...
/* ---------- carrusel ---------- */
var EST=[14000,10000,20000], HOLD=5000;   // Warren y Portfolio duran lo que tardan en escribirse + 5 s de pausa; Informes 10 s
var track=root.getElementById('track'), tabs=Array.prototype.slice.call(root.querySelectorAll('.tab'));
var cur=-1, elapsed=0, finishedAt=null, hover=false, visible=false, last=0;
function go(i){
  if(cur===0)stopWarren(); if(cur===1)deckStop(); if(cur===2)stopPortfolio();
  cur=i; elapsed=0; finishedAt=null;
  ...
  setTimeout(function(){
    if(cur!==i)return;
    var done=function(){ if(cur===i&&finishedAt===null)finishedAt=elapsed; };
    if(i===0)runWarren().then(done); if(i===1)deckStart(); if(i===2)runPortfolio().then(done);
  },fixed?100:750);
}
...
function tick(ts){
  ...
  if(!fixed && visible && !hover && cur>=0){
    elapsed+=dt;
    var total, adv;
    if(cur===1){total=10000; adv=elapsed>=total;}
    else if(finishedAt!==null){total=finishedAt+HOLD; adv=elapsed>=total;}
    else {total=EST[cur]+HOLD; adv=false;}
    tabs[cur].querySelector('b').style.width=Math.min(finishedAt!==null||cur===1?100:96,elapsed/total*100)+'%';
    if(adv) go((cur+1)%3);
  }
...
new IntersectionObserver(function(es){es.forEach(function(e){
  var was=visible; visible=e.intersectionRatio>=.4;
