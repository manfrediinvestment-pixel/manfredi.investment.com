...
var WS={v:0,k:4};   // k = factor de velocidad: el analisis completo se escribe en ~3 s
...
function deckStart(){head=0; cards.forEach(function(c){c.classList.remove('out');}); layout(true); clearInterval(deckTimer); deckTimer=setInterval(step,1300);}
...
var PS={v:0,k:6};   // k = factor de velocidad: la carga de 5 activos + ratios en ~3,5 s
...
/* ---------- carrusel ---------- */
var DUR=5000;   // cada seccion dura 5 s y pasa a la siguiente
